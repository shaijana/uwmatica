...
public class SchematicProject {
    ...
    public SchematicProject(final File directory, final File projectFile) {
    ...
    public String getCurrentVersionName() {
        final SchematicVersion currentVersion = this.getCurrentVersion();
        return currentVersion != null ? currentVersion.getName() : this.getSelection().getName();
    }

    public void setName(final String name) {
        final File newFile = new File(this.directory, name + ".json");
...
//SH                SelectionManager.renameSubRegionBoxIfSingle(this.selectionSimple, name);
    ...
    public void setOrigin(final BlockPos origin) {
        ...
        if (currentVersion != null) {
            final BlockPos areaPosition = this.origin.add(currentVersion.getAreaOffset());
    ...
    private void createAndAddPlacement() {
        ...
        if (version != null) {
            if (this.currentVersionId != this.lastCheckedOutVersion) {
                ...
                final LitematicaSchematic schematic = LitematicaSchematic.createFromFile(this.directory, version.getFileName());

                if (schematic != null) {
                    final BlockPos areaPosition = this.origin.add(version.getAreaOffset());
                    ...
                    final long time = schematic.getMetadata().getTimeCreated();
...
/*SH    public void deleteLastSeenArea(MinecraftClient mc)
    ...
    public boolean switchVersion(final int version, final boolean createPlacement) {
    ...
    public boolean switchVersion(final SchematicVersion version, final boolean createPlacement) {
        final int index = this.versions.indexOf(version);
    ...
    public boolean commitNewVersion(final String name) {
        if (this.checkCanSaveOrPrintError()) {
            final MinecraftClient mc = MinecraftClient.getInstance();
            final String author = mc.player.getName().getString();
            final String fileName = this.getNextFileName();

            final AreaSelection selection = this.getSelection();
            final LitematicaSchematic schematic = LitematicaSchematic.createEmptySchematic(selection, author);
            schematic.getMetadata().setName(name);
            final BlockPos areaOffset = selection.getEffectiveOrigin().subtract(this.origin);
            final SaveCompletionListener listener = new SaveCompletionListener(name, fileName, areaOffset);
            final LitematicaSchematic.SchematicSaveInfo info = new LitematicaSchematic.SchematicSaveInfo(false, false);

            final TaskSaveSchematic task = new TaskSaveSchematic(this.directory, fileName, schematic, selection.copy(), info, false);
    ...
    private String getNextFileName() {
        final String nameBase = this.projectName + "_";
        int version = 1;
        int failsafe = 10000000;

        while (failsafe-- > 0) {
            final String name = nameBase + String.format("%05d", version);
            final File file = new File(this.directory, name + LitematicaSchematic.FILE_EXTENSION);
    ...
    @Nullable
    public static SchematicProject fromJson(final JsonObject obj, File projectFile, final boolean createPlacement) {
        final BlockPos origin = JsonUtils.blockPosFromJson(obj, "origin");
        ...
        if (JsonUtils.hasString(obj, "name") && JsonUtils.hasInteger(obj, "current_version_id") && origin != null) {
            ...
            final SchematicProject project = new SchematicProject(projectFile.getParentFile(), projectFile);
            ...
            if (JsonUtils.hasArray(obj, "versions")) {
                final JsonArray arr = obj.get("versions").getAsJsonArray();

                for (int i = 0; i < arr.size(); ++i) {
                    final JsonElement el = arr.get(i);

                    if (el.isJsonObject()) {
                        final SchematicVersion version = SchematicVersion.fromJson(el.getAsJsonObject());
            ...
            if (JsonUtils.hasInteger(obj, "current_version_id")) {
                final int tmp = JsonUtils.getInteger(obj, "current_version_id");
    ...
    private class SaveCompletionListener implements ICompletionListener {
        ...
        private SaveCompletionListener(final String name, final String fileName, final BlockPos areaOffset) {
            this.name = name;
        ...
        private void saveVersion() {
            final SchematicVersion version = new SchematicVersion(this.name, this.fileName, this.areaOffset, this.version, System.currentTimeMillis());
